...
sanger1st$taxo <- fifelse(grepl("Eucyrtidium", sanger1st$group), "Nassellaria", 
                          fifelse(grepl("Triastrum|Heliodiscus|Styptosphaera|Spongodiscidae|Euchitonia|Rhizosphaera", sanger1st$group), "Spumellaria", (
...
write.table(sanger1st, "data/fig2_sanger_toCHECK.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)
...
# Write table
write.table(minion1st, "data/fig2_minion_toCHECK.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)
...
write.table(illuOut, "data/fig2_illumina.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)


#----
#---- Creating a single file with all the information for plotting ---------------------------------

tmps <- fread("data/fig2_sanger.tsv", sep="\t")
tmpm <- fread("data/fig2_minion.tsv", sep="\t")
tmpi <- fread("data/fig2_illumina.tsv", sep="\t")
...
write.table(file, "data/fig2.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)
...
(treemap <- ggplot(file, aes(area=reads, fill=taxo))+
  geom_treemap()+
  facet_grid(cell~method, space="free")+
  geom_treemap_text(aes(label= paste(taxo, reads, sep="\n")), place = "centre")+
  scale_fill_manual(values=color)+
  theme(legend.position="none"))

pdf(paste("plots/Fig2_Treemaps.pdf", sep=""), width=8.27, height=11.69, paper='special')
plot(treemap)
dev.off()
