...
TAGS="data/minion/info/info_demultiplex.txt"
...
for FILE in $(ls minion/demultiplexed/*fasta)
do
	PREF=${FILE/.fasta/_}
	sed -e "s\>\>$PREF\g" $FILE >> minion/raw/demultiplexed.fasta
done
sed -i 's\data/minion/demultiplexed/\\g' minion/raw/demultiplexed.fasta

# setting BLASTn parameters
FILE="data/minion/demultiplexed.fasta"
BLAST_TSV="data/minion/demultiplexed_blast.tsv"
...
DB="/media/Data_1/Miguel/DB/NCBI/"
DBnt="$DB/nt"
