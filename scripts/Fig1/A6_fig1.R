...
sanger1st <- sanger1st %>% separate(qseqid, c("cell", "primer", "replicate", "well"), sep="_", remove=FALSE)
...
#write.table(demultiplexed, "data/files/minion_demultiplexed.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)
...
file$species <- fifelse(file$cell=="Mge17-9", "Ex. undulatum", 
                        fifelse(file$cell=="Mge17-124",  "C. obliqua", 
                                fifelse(file$cell=="Vil325",   "Eu. cienkowski", 
                                        fifelse(file$cell=="Vil490",   "P. zanclea", 
                                                fifelse(file$cell=="Vil496",   "Eu. acuminatum", 
                                                        fifelse(file$cell=="Mge17-81",  "R. trigonacantha",  
                                                                fifelse(file$cell=="Mge17-82",  "S. streptacantha",  
                                                                        fifelse(file$cell=="Vil480",   "T. octacantha", 
                                                                                fifelse(file$cell=="Vil497", "A. varians", "NA")))))))))
...
if(!dir.exists("data/files")){dir.create("data/files")}

write.table(file, "data/files/fig1.tsv", quote=FALSE, sep='\t', row.names=FALSE, col.names=TRUE)
...
(boxplots <- ggplot(file, aes(x=species, y=value, fill=order))+
  geom_boxplot()+
...
pdf(paste0("plots/Fig1.pdf", sep=""), width=11.69, height=8.27, paper='special')
plot(boxplots)
dev.off()
