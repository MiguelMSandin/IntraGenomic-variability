...
file <- file %>% group_by(cell, query, taxo) %>% summarise(value=sum(value))
...
treemap <- ggplot(file, aes(area=value, fill=taxo))+
  ...
  geom_treemap_text(aes(label= paste(taxo, query, sep="\n")), place = "centre")+
  scale_fill_manual(values=color)+
  theme(legend.position="none")
treemap

pdf(paste("../plots/FigS3_Treemaps_abun.pdf", sep=""), width=8.27, height=11.69, paper='special')
plot(treemap)
dev.off()
