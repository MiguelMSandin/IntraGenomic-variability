...
setwd("/home/mmsandin/Desktop/PhD/0_Thesis/3.1_Chapter/IntraGenomic-variability/data/sanger/alignments/")
...
for(j in positions){
    for(i in 1:nrow(data)){
        ...
        seqs[i] <- ifelse(grepl("^-*$", seqs[i]), seqs[i], gsub("-", "N", seqs[i]))
    }
    (logo <- ggplot() + 
            geom_logo(seqs, method='p', namespace='ACTGN') + 
            theme_logo() +
